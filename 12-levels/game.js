...
var enemies = {
    ...
    ltr:      { x: 25,   y: -100, sprite: 'enemy_purple', health: 10, 
		B: 75, C: 10, E: 180  },
    ...
    circle:   { x: 250,   y: -50, sprite: 'enemy_circle', health: 10, 
		A: 0,  B: -100, C: 1, E: 50, F: 10, G: 1, H: Math.PI/2 },
...
var midGame = function() {
    ...
    Game.setBoard(1,new Starfield(50,0.6,100));
    Game.setBoard(2,new Starfield(100,1.0,50));
    Game.setBoard(3,new TitleScreen("Level 2", 
                                    "Press fire to start Level 2",
                                    playGame2));
};
...
var level2 = [
  //  Comienzo, Fin,   Frecuencia,  Tipo,       Override
    ...
    [ 2000,     13000, 800,         'ltr'                  ],
	[ 3000,     13000, 350,         'ltr'                  ],
    [ 6000,     16000, 400,         'circle'               ],
    [ 10000,    16000, 40,         'circle'               ],
    [ 17800,    20000, 500,         'straight', { x: 50  } ],
    [ 7200,    	20000, 500,         'straight', { x: 90  } ],
    [ 8200,    	20000, 500,         'straight', { x: 10  } ],
    [ 15200,    20000, 50,         'straight', { x: 90  } ],
    [ 18200,    20000, 500,         'straight', { x: 10  } ],
    [ 22000,    25000, 4,         'wiggle',   { x: 150 } ],
    [ 22000,    25000, 4,         'wiggle',   { x: 100 } ]
];
...
var playGame = function() {
    ...
    // ganado el juego
    board.add(new Level(level1, midGame));
...
var PlayerShip = function() { 
	...
	if(Game.keys['n'] && this.reload < 0) {
		this.reload = this.reloadTime;
		this.board.add(new FireBall(this.x+40,this.y+this.h/2,1));
	}
	...
	if(bool && Game.keys['fire'] && this.reload < 0) {
	    // Esta pulsada la tecla de disparo y ya ha pasado el tiempo reload
	    
	    // Aqui cuando ya ha disparado ponemos que no puede disparar.
	    bool = false;
	    this.reload = this.reloadTime;
...
var FireBall = function(x,y,side){
	this.setup('fireball',{vy:-600,vx:60*side,damage: 20});
	this.x = x - this.w/2; 
    this.y = y - this.h;
};
...
FireBall.prototype.step = function(dt)  {
    ...
    if(collision) {
		collision.hit(this.damage); 
    } else if(this.y > Game.height || this.y < -this.h|| this.x < -this.w|| this.x > Game.width) {
		this.board.remove(this);
    }
};
